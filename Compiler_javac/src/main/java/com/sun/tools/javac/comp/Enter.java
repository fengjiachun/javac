 ...
 *************************************************************************/
    ...
    public void visitTopLevel(JCCompilationUnit tree) {
        ...
        /*
         * package-info类，它是专门为本包服务的
         * 1.它不能随便被创建
         *          在一般的IDE中，Eclipse、package-info等文件是不能随便被创建的，
         *          会报“Type name is notvalid”错误，类名无效。在Java变量定义规范中规定如下字符是允许的：
         *          字母、数字、下划线，以及那个不怎么常用的$符号，不过中划线不在之列
         *          创建这个文件的方法：别处创建好拷贝过来
         * 2.它服务的对象很特殊
         *          它是描述和记录本包信息的
        ...
        if (tree.pid != null) {     // [tree.pid]: The tree representing the package clause. 包名
            // Make a package
            tree.packge = reader.enterPackage(TreeInfo.fullName(tree.pid));
            if (tree.packageAnnotations.nonEmpty() || pkginfoOpt == PkgInfo.ALWAYS) {   // -Xpkginfo:{always, legacy, nonempty} 指定 packeg-info 文件的处理
                if (isPkgInfo) {
                    addEnv = true;
                } else {
