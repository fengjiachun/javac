...
public class RecognizedOptions {
    ...
    // 动态编译， JavacTools用的到，不准调用任何OptionHelper方法
    ...
    public static Option[] getAll(final OptionHelper helper) {
        // com.sun.tools.javac.main.Main.processArgs()会调用 process()方法
        return new Option[] {
        new Option(G,                                           "opt.g"),
        new Option(G_NONE,                                      "opt.g.none") {
            // 继承自Option并重写了process(Options, option)方法的匿名内部类
            @Override
    ...
    public enum PkgInfo {
        ...
        public static PkgInfo get(Options options) {
            String v = options.get(XPKGINFO);   // -Xpkginfo:{always, legacy, nonempty} 指定 packeg-info 文件的处理
            return (v == null
                    ? PkgInfo.LEGACY
