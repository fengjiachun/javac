 ...
 *  
 *  语法分析:
 *  词法分析其实是做为语法分析阶段的一个过程(或方法)来驱动的，
 *  整个语法分析阶段完成的任务就是按照java语言的LL(1)文法采用递归下降算法并结合运算符优先规则，
 *  对token序列构造一棵抽象语法树(abstract syntax tree)
 *
...
public class JavacParser implements Parser {
    ...
    protected JavacParser(ParserFactory fac,
                     ...
                     boolean keepLineMap) {
        this.S = S;
        S.nextToken(); // prime the pump 定位到第一个token
        this.F = fac.F;
        this.log = fac.log;
...
/* ---------- error recovery -------------- */
     ...
     *  an error.
     *  校验当前的token，如果符合预期，则定位到下一个token，否则报错
     */
...
/* ---------- parsing -------------- */
    ...
    /**
     * Ident = IDENTIFIER 解析当前Token的Name，并定位到下一个Token
     */
    Name ident() {
        if (S.token() == IDENTIFIER) {
            ...
            S.nextToken();
            return name;
        } else if (S.token() == ASSERT) {   // ASSERT   "assert"
            if (allowAsserts) {
        ...
        } else if (S.token() == ENUM) {     // ENUM     "enum"
            if (allowEnums) {
     ...
     * 解析带"."的
     * 例如解析如下表达式com.javac.test产生如下结构的一棵树：
     *              (JCFieldAccess)com.javac.test
     *                  /                      \
     *      (JCFieldAccess)com.javac       test(Name)
     *               /             \
     *      (JCIdent)com       javac(Name)
     *              \
     *          com(Name)
     */
    public JCExpression qualident() {
        JCExpression t = toP(F.at(S.pos()).Ident(ident()));
        while (S.token() == DOT) {  // DOT  "."
            int pos = S.pos();
            S.nextToken();
...
//where
    ...
    JCExpression typeArgument() {
        if (S.token() != QUES) return parseType();  // QUES("?")
        int pos = S.pos();
        S.nextToken();
        if (S.token() == EXTENDS) {
     ...
     */
    public JCExpression variableInitializer() { // LBRACE("{") 如果当前Token是"{"就要执行数组初始化处理
...
//todo: skip to anchor on error(?)
    ...
    loop:
        while (true) {
            long flag;
            switch (S.token()) {
            case PRIVATE     : flag = Flags.PRIVATE; break;         // 00000000000010
            case PROTECTED   : flag = Flags.PROTECTED; break;       // 00000000000100
            case PUBLIC      : flag = Flags.PUBLIC; break;          // 00000000000001
            case STATIC      : flag = Flags.STATIC; break;          // 00000000001000
            case TRANSIENT   : flag = Flags.TRANSIENT; break;       // 00000010000000
            case FINAL       : flag = Flags.FINAL; break;           // 00000000010000
            case ABSTRACT    : flag = Flags.ABSTRACT; break;        // 00010000000000
            case NATIVE      : flag = Flags.NATIVE; break;          // 00000100000000
            case VOLATILE    : flag = Flags.VOLATILE; break;        // 00000001000000
            case SYNCHRONIZED: flag = Flags.SYNCHRONIZED; break;    // 00000000100000
            case STRICTFP    : flag = Flags.STRICTFP; break;        // 00100000000000
            case MONKEYS_AT  : flag = Flags.ANNOTATION; break;      // 10000000000000
            default: break loop;
            }
            if ((flags & flag) != 0) error(S.pos(), "repeated.modifier");   // 修饰符重复
            lastPos = S.pos();
            S.nextToken();
                                                                     ...
                                                                     T vdefs)
    {
        ...
        while (S.token() == COMMA) {    // COMMA(",")
            /*
             * 当前token是","(比如String s1, s2)
             * 这是只有接着调用variableDeclarator的另一个重载方法
             * 这个方法里name用ident()取得
             */
    ...
    JCVariableDecl variableDeclaratorRest(int pos, JCModifiers mods, JCExpression type, Name name,
        ...
        type = bracketsOpt(type);           // 数组
        JCExpression init = null;
        if (S.token() == EQ) {              // EQ   "="
            S.nextToken();
            init = variableInitializer();   // 变量初始化
        }
    ...
    public JCTree.JCCompilationUnit parseCompilationUnit() {
        int pos = S.pos();          // pos: 当前Token的开始位置
        JCExpression pid = null;    // package的树
        String dc = S.docComment();
        ...
        if (S.token() == PACKAGE) {     // package 解析package
            if (mods != null) {
            ...
            S.nextToken();
            pid = qualident();      // Qualident = Ident { DOT Ident } 解析带"."的
            accept(SEMI);           // package是必须要以;结尾的
        }
        ListBuffer<JCTree> defs = new ListBuffer<JCTree>();
        boolean checkForImports = true;
        while (S.token() != EOF) {  // 单个源文件的所有Token将在这个循环体内都解析完毕
            if (S.pos() <= errorEndPos) {
                // error recovery
            ...
            if (checkForImports && mods == null && S.token() == IMPORT) {   // import
                defs.append(importDeclaration());   // ImportDeclaration = IMPORT [ STATIC ] Ident { "." Ident } [ "." "*" ] ";"
            } else {
                //def：一个类、接口或枚举的树
    ...
    JCTree importDeclaration() {
        ...
        if (S.token() == STATIC) {  // 静态引用 import static...
            checkStaticImports();
            importStatic = true;
            S.nextToken();
        }
    ...
    JCTree typeDeclaration(JCModifiers mods) {
        int pos = S.pos();  // 基于0的偏移，当前Token的开始位置
        if (mods == null && S.token() == SEMI) {    // SEMI --> ;
            S.nextToken();
            return toP(F.at(pos).Skip());
        } else {
            String dc = S.docComment();
     ...
     *  Class、Interface、Enum 声明的修饰符(public、private等)
     *  
    ...
    JCClassDecl classDeclaration(JCModifiers mods, String dc) {
        int pos = S.pos();
        // 判断当前的token是不是class，是的话定位到下一个token
        accept(CLASS);
        // 取得当前token（就是类名称），然后定位到下一个token
        Name name = ident();

        List<JCTypeParameter> typarams = typeParametersOpt();   // 处理类的泛型参数

        JCExpression extending = null;
        if (S.token() == EXTENDS) {     // EXTENDS  "extends"       当前类的继承关系
            S.nextToken();
            extending = parseType();
        }
        List<JCExpression> implementing = List.nil();
        if (S.token() == IMPLEMENTS) {  // IMPLEMENTS  "implements" 当前类实现的接口
            S.nextToken();
            implementing = typeList();
        }
        // defs:类、接口的变量树、方法树的列表
        // 根据类名构造整个类的抽象语法树
    ...
    List<JCTree> classOrInterfaceBody(Name className, boolean isInterface) {
        ...
        ListBuffer<JCTree> defs = new ListBuffer<JCTree>();
        // 只要没有遇到"}"或者token序列结束，就证明这个类没解析完，一直循环处理
        while (S.token() != RBRACE && S.token() != EOF) {   // RBRACE   "}"
            // classOrInterfaceBodyDeclaration:返回一个变量或方法的树
        ...
        accept(RBRACE);     // 预期当前Token是RBRACE   "}"
        return defs.toList();
    }
    ...
    List<JCTree> classOrInterfaceBodyDeclaration(Name className, boolean isInterface) {
        if (S.token() == SEMI) {    // SEMI     ";"
            ...
            int pos = S.pos();      // 基于0的偏移，当前Token的开始位置
            JCModifiers mods = modifiersOpt();
            if (S.token() == CLASS ||               // 内部类
                S.token() == INTERFACE ||           // 内部接口
                allowEnums && S.token() == ENUM) {  // 内部枚举
                return List.<JCTree>of(classOrInterfaceOrEnumDeclaration(mods, dc));
            } else if (S.token() == LBRACE && !isInterface &&   // LBRACE("{")
                ...
                if (S.token() == LPAREN && !isInterface && type.getTag() == JCTree.IDENT) { // LPAREN   "("
                    ...
                    pos = S.pos();
                    name = ident();
                    if (S.token() == LPAREN) {                  // LPAREN   "(" Method，方法都有左括号
                        //  返回方法的语法树
                        return List.of(methodDeclaratorRest(
                            pos, mods, type, name, typarams,
                            isInterface, isVoid, dc));
                    } else if (!isVoid && typarams.isEmpty()) { // variable
                        //  定义实例变量的语法树
                        List<JCTree> defs =
                            variableDeclaratorsRest(pos, mods, type, name, isInterface, dc,
                                                    new ListBuffer<JCTree>()).toList();
                        storeEnd(defs.last(), S.endPos());
                        accept(SEMI);   // 预期当前的token是";"
                        return defs;
                    } else {
                        pos = S.pos();
    ...
    JCTypeParameter typeParameter() {
        int pos = S.pos();
        Name name = ident();    // 解析当前Token的Name
        ListBuffer<JCExpression> bounds = new ListBuffer<JCExpression>();
        if (S.token() == EXTENDS) {     // EXTENDS("extends") 泛型参数类型限制
            S.nextToken();
            bounds.append(parseType());
            while (S.token() == AMP) {  // AMP("&")
                S.nextToken();
                bounds.append(parseType());
            }
        }
