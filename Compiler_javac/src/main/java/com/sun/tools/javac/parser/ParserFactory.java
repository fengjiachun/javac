...
public class ParserFactory {
    ...
    public Parser newParser(CharSequence input, boolean keepDocComments, boolean keepEndPos, boolean keepLineMap) {
        // 先建立一个词法分析实例,并指向第一个字符
        // DocCommentScanner或Scanner
        Lexer lexer = scannerFactory.newScanner(input, keepDocComments);
        /*
         * keepEndPos == JavaCompiler.genEndPos,下面代码来自JavaCompiler.java:
         * genEndPos     = options.isSet(XJCOV) ||
                        context.get(DiagnosticListener.class) != null;
         * 根据我目前的测试当要做注解处理的时候genEndPos = true，
         * 这在JavaCompiler.initProcessAnnotations方法中有对应的处理
         */
        if (keepEndPos) {
            // 当有注解处理器或javac -Xjcov的时候，建立EndPosParser语法分析实例，并定位到第一个token
            return new EndPosParser(this, lexer, keepDocComments, keepLineMap);
        } else {
            // 建立语法分析实例，并定位到第一个token
