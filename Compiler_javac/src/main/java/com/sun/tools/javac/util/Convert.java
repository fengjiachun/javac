 ...
 */
     ...
     *  @param src        The array holding the characters to convert. 待转换的数组
     *  @param sindex     The start index from which characters are converted.
     *  @param dst        The array holding the converted characters.. 维持着转换后的数组（不删除之前的数据）
     *  @param dindex     The start index from which converted bytes
     *                    are written.
     *  @param len        The maximum number of characters to convert. 待转换数组的长度
     */
    public static int chars2utf(char[] src, int sindex,
        ...
        for (int i = sindex; i < limit; i++) {
            char ch = src[i];
            if (1 <= ch && ch <= 0x7F) {    // 1---127 字母 : 一个字节
                dst[j++] = (byte)ch;
            } else if (ch <= 0x7FF) {       // 128---7*16*16 + 15*16 + 15 : 两个字节
                dst[j++] = (byte)(0xC0 | (ch >> 6));
                dst[j++] = (byte)(0x80 | (ch & 0x3F));
            } else {                        // 最大的情况(如一个中文字符)要占3个字节
