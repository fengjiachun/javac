...
public class Names {
    ...
    public Names(Context context) {
        ...
        table = createTable(options);  // com.sun.tools.javac.util.SharedNameTable

        // 将以下字符串构造成Name(一般是SharedNameTable$NameImpl)，同时也存储在SharedName#bytes的字节数组中
        slash = fromString("/");
        hyphen = fromString("-");
        T = fromString("T");
        ...
        init = fromString("<init>");        // 实例初始化
        clinit = fromString("<clinit>");    // 类初始化
        ...
        LineNumberTable = fromString("LineNumberTable");                    // 行号信息
        LocalVariableTable = fromString("LocalVariableTable");              // 局部变量表
